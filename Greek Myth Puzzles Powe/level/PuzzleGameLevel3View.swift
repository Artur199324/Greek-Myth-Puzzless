...
struct PuzzleGameLevel3View: View {
    @State private var imagePositions: [CGPoint] = Array(repeating: .zero, count: 6)
    @State private var isFullScreenPresented = false
    @State private var restartViewID = UUID()

    @State private var imgStates: [Bool] = Array(repeating: false, count: 6)
                ...
                VStack {
                    ...
                    VStack(spacing: geometry.size.height * 0.02) {
                        HStack(spacing: geometry.size.width * 0.02) {
                            ...
                            puzzleTargetView(index: 2, geometry: geometry)
                        }

                        HStack(spacing: geometry.size.width * 0.02) {
                            puzzleTargetView(index: 3, geometry: geometry)
                            VStack(spacing: geometry.size.height * 0.02) {
                                puzzleTargetView(index: 4, geometry: geometry)
                                puzzleTargetView(index: 5, geometry: geometry)
                            }
                        }
                    }
    ...
    func puzzleTargetView(index: Int, geometry: GeometryProxy) -> some View {
        Image(!imgStates[index] ? "blue_square" : puzzleParts[index])
            .resizable()
            .aspectRatio(contentMode: .fit)
            .frame(width: geometry.size.width * 0.2, height: index < 2 ? geometry.size.width * 0.1 : geometry.size.width * 0.2)
            .background(
                GeometryReader { geo in
                    Color.clear
                        .onAppear {
